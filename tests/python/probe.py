...
class TestProbe(unittest.TestCase):
    ...
    def test_uprobe(self):
        regex = "a: 10, b: 20"
        cmd = ("test=uprobe; sleep 1 && ./pro.exe & {} -e ".format(environ["BPFTRACE_RUNTIME"]) +
        "'uprobe:./pro.exe:somefunc {printf(\"a: %d, b: %d\", arg0, arg1); exit();}'" +
        " | grep '{}' || echo \"FAILURE $test\"".format(regex))
        Utils.run_test(self, cmd, regex)

    def test_uretprobe(self):
        regex = "ret: 30"
        cmd = ("test=uretprobe; sleep 1 && ./pro.exe & {} -e ".format(environ["BPFTRACE_RUNTIME"]) +
        "'uretprobe:./pro.exe:somefunc {printf(\"ret: %d\", retval); exit();} '" +
        " | grep '{}' || echo \"FAILURE $test\"".format(regex))
        Utils.run_test(self, cmd, regex)
